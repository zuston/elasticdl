...
class Worker(object):
    ...
    def _init_callbacks(self, args):
        if not self._args.need_task_manager:
            return
    ...
    def _train_and_evaluate(self):
        ...
        dataset = None
        if self._args.need_task_manager:
            # 从 task data service 获取到对应的 dataset
            ...
            dataset = self._getStaticShard()[0]
            # 先直接写死，再改造下
            # dataset = self._feed(
            #     dataset,
            #     Mode.TRAINING,
        ...
        for step, batchData in enumerate(dataset):
    ...
    def _getStaticShard(self):
        """
        返回一个根据当前 worker 数目的静态切分数据的方法
        """
        worker_number = self._args.num_workers
        worker_id = int(self._args.worker_id)
        training_dataset = self._args.training_data

        print("work number is " + str(worker_number))
        print("work id is " + str(worker_id))

        # dataset = tf.data.Dataset.from_tensor_slices([training_dataset])
        #
        # dataset = dataset.shard(worker_number, worker_id - 1)
        ...
        # )
        # dataset = dataset.repeat(self._args.num_epochs).shuffle(
        #     self._minibatch_size * 20,
        ...
        train_dataset = train_dataset.shuffle(buffer_size=1024).batch(self._minibatch_size)
